...
// Global Varibles 
const int MAX_INPUT_LEN = 16; //Task 1,2,3
...
// Task 1
void cardToNumbers(char chars[], int length, int cards_num[]);
// Task 2
        ...
        if(cards[i]=='*'){
            ...
            switch(cards[i]){
                ...
                case 'K':
                    ...
                    break;
                default:
                    cerr<<"undefined cards string"<<endl;
            }
        }
    }
}
...
// Task 3
// core function for continuity check
bool isContinuousCore(int cards_num[], int length){
    int numOfZero = 0;
    int numOfGap = 0;

    //count number of zeros
    for(int i=0;i<length&&cards_num[i]==0;++i)
        ++numOfZero;
    ...
    int large = small+1;
    while(large<length){
        if(cards_num[small]==cards_num[large])
            return false;
        
        numOfGap += cards_num[large] - cards_num[small] -1;
        small = large;
        ++large;
    }
...
bool isContinuous(char cards[], int length, int cards_num[]){
    ...
    */
    ...
    bool result = isContinuousCore(cards_num, length);

    // check for special case, at this time 
	bool changeA = false;
    for(int i=0;i<length;++i){
        if(cards_num[i]==1){
            cards_num[i]=14;
    ...
    sortCards(cards_num, length);
    bool special_check = isContinuousCore(cards_num, length);
	if(changeA && !special_check){
    ...
    return (result || special_check);
}

// Task 4
int checkValue(int sequence[], const int length, bool isStraight){
    // sequence is sorted
    ...
    if(isStraight){
        ...
        if(numOfZero==0){
            ...
            for(int i=numOfZero;i<length-1;++i){
                numOfZero -= sequence[i+1] - sequence[i] -1;
            }
            if(numOfZero>0){
                value = ((sequence[length-1] + numOfZero)>=14) ? 14 : sequence[length-1] + numOfZero;
            }
            else{
                value = sequence[length-1];
            }
...
int compareValue(char handA[], char handB[], int& valueA, int& valueB){
   ...
   int handA_num[VALUE_COMPARE_LEN], handB_num[VALUE_COMPARE_LEN];
   bool handA_check = isContinuous(handA, VALUE_COMPARE_LEN, handA_num);
   bool handB_check = isContinuous(handB, VALUE_COMPARE_LEN, handB_num);
   valueA = checkValue(handA_num, VALUE_COMPARE_LEN, handA_check);
   valueB = checkValue(handB_num, VALUE_COMPARE_LEN, handB_check);
   
   if(handA_check && !handB_check){
       return 1;
   }
   else if(!handA_check && handB_check){
       return -1;
   }
   else{
       if(valueA > valueB){
...
// Task 5
int longestStraightCore(int cards_num[]){
    int max_length=0;
    int start_index=0;
    // find the longest straight's length and start index
    for(int i=0;i<INPUT_STRAIGHT_LEN;++i){
        int cur_len = 1;
        for(int j=i+1;j<INPUT_STRAIGHT_LEN;++j){
            if(cards_num[j]-cards_num[j-1]==1){
                ++cur_len;
            }
            else if(cards_num[j]==cards_num[j-1]){
                continue;
            }
            else{
                break;
        ...
        if(cur_len>=max_length && cur_len>=MIN_STRAIGHT_LEN){
            max_length = cur_len;
            start_index = i;
        }
    }
    // if there is no straight whose length is longer than MIN_STRAIGHT_LEN
    if(max_length==0){
        return 0;
    }

    // Use an array to set the element which is used to 1.
    int cards_flag[INPUT_STRAIGHT_LEN];
    int new_cards_num[INPUT_STRAIGHT_LEN];
    for(int i=0;i<INPUT_STRAIGHT_LEN;++i){
        cards_flag[i]=0;
        new_cards_num[i]=0;
    }
    int count = 0;
    for(int i=start_index; count<max_length; ++i){
        if(i==start_index || (cards_num[i] - cards_num[i-1])==1){
            ++count;
            cards_flag[i] = 1;
        }
    }
    ...
    while(index<max_length){
        for(; cards_flag[start_index]==0; ++start_index);
        new_cards_num[index++] = cards_num[start_index++];
    }
    for(int i=0;i<INPUT_STRAIGHT_LEN;++i){
        if(cards_flag[i]==0){
            new_cards_num[index++] = cards_num[i];
        }
    }

    // Assign the sorted array to original array
    for(int i=0; i<INPUT_STRAIGHT_LEN; ++i){
        cards_num[i] = new_cards_num[i];
    }
    return max_length;
}
...
int longestStraight(int cards_num[]){
    /*
        ...
        The length of the input array is fixed as MIN_STRAIGHT_LEN.
        Parameters:
    ...
    */
    int temp_array[INPUT_STRAIGHT_LEN];
    for(int i=0; i<INPUT_STRAIGHT_LEN;++i){
        temp_array[i] = cards_num[i];
    }
    sortCards(temp_array, INPUT_STRAIGHT_LEN);
    int max_length1 = longestStraightCore(temp_array);

    if(cards_num[0]==1){
        cards_num[0]=14;
        sortCards(cards_num, INPUT_STRAIGHT_LEN);
        int max_length2 = longestStraightCore(cards_num);
        if(max_length2>=max_length1 && cards_num[max_length2-1]==14){
            return max_length2;
        }
    }
    for(int i=0; i<INPUT_STRAIGHT_LEN;++i){
        cards_num[i] = temp_array[i];
    }
