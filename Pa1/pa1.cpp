...
void printNums2(int cards_num[], int length)
{
...
bool isContinuous(char cards[], int length, int cards_num[])
{
    ...
    */
    ...
    for (int i = 0; i < length; i++)
    {
        if (cards_num[i] + 1 == cards_num[i + 1] || cards_num[i + 1] == 0 || cards_num[i] == 0 || i == length - 1)
        {
            if (cards_num[i] == 0)
            {
                jokerCount++;
